...
class GenerationServer:
    ...
    def __init__(self, hostname="", http_port=80, sqlite_path='/opt/dcept/var/honeytoken.db'):
        ...
        if not config.master_node:
            ...
            logging.info("Database contains %d generated passwords" %
                         self.getRecordCount())
            ...
            thread.daemon = True
            logging.info("Starting honeytoken generation server HTTP daemon %s:%d" % (
                hostname, http_port))
            thread.start()
    ...
    def initDatabase(self):
        if not os.path.exists(self.sqlite_path):
            ...
            c = self.conn.cursor()
            c.execute(
                '''CREATE TABLE db_version (major integer, minor integer)''')
            ...
            ), "ALLSAFE.LAN", "Administrator", "FAKE-PC", "dcepttest"))
            self.conn.commit()
        else:
            self.conn = sqlite3.connect(
                self.sqlite_path, check_same_thread=False)
    ...
    def do_GET(s):
      ...
      # Log transaction
        ...
        c.execute("INSERT INTO logs VALUES (?,?,?,?,?)",
                  (datetime.now(), domain, username, machine, password))
