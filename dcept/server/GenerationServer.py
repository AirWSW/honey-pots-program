...
class GenerationServer:
    ...
    def initDatabase(self):
        if not os.path.exists(self.sqlite_path):
            ...
            c.execute('''CREATE TABLE logs (date text, domain text, username text, machineip text, machine text, password text)''')

            # Add a test honeytoken to the database
            c.execute("INSERT INTO logs VALUES (?,?,?,?,?,?)", (datetime.now(), config.domain, config.honey_username, "0.0.0.0", "FAKE-PC", "dcepttest"))
            self.conn.commit()
        else:
            self.conn = sqlite3.connect(self.sqlite_path, check_same_thread=False)
    ...
    def genPass(self):
        ...
        while True:
            ...
            logging.info("Generated - %s" % (password))
...
class HttpHandler(BaseHTTPRequestHandler):
    ...
    def do_GET(s):
        ...
        try:
            ...
            return
        logging.info("Request from IP: %s, workstation: %s" % (s.client_address[0], machine))
        ...
        logging.info("Sent: %s" % (jSONstring))

        # Log transaction
        c = gsHandle.conn.cursor()
        c.execute("INSERT INTO logs VALUES (?,?,?,?,?,?)", (datetime.now(), domain, username, s.client_address[0], machine, password))
    ...
    def do_POST(s):
        ...
        try:
            ...
            hostName     = postvars['h'][0]
            srcAddress   = postvars['s'][0]
        except:        
            s.send_response(500)
            s.end_headers()    
            return

        cracker.enqueueJob(username, domain, encTimestamp, [hostName, srcAddress], dcept.passwordHit)
