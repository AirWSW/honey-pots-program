...
def kerbsniff(interface, username, domain, realm):
    ...
    logging.info("kerbsniff: Looking for %s\%s on %s" %
                 (domain, username, interface))
...
def notifyMaster(username, domain, encTimestamp):
    ...
    try:
        ...
        message = "DCEPT slave Failed to communicate with master node '%s'" % (
            config.master_node)
...
def kerb_handler(kp, domain, username):
    ...
    if kp.msg_type == "10":
        ...
        # logging.info("kerberos: %s" % kp)
        # logging.info("kerberos.CNameString: %s" % kp.CNameString)
        ...
        if kerbName.lower() == username.lower() and realm.lower() == config.realm.lower():
            ...
            logging.debug("Ignoring kerb-as-req for '%s\%s'" %
                          (realm, kerbName))
...
"""
    ...
    if not testInterface(config.interface):
        logging.error("Unable to listen on '%s'. Is the interface up?" %
                      (config.interface))
        raise DceptError()
    ...
    if not config.master_node:  # (Master Node)
        ...
        try:
            global genServer
            genServer = GenerationServer.GenerationServer(
                config.honeytoken_host, config.honeytoken_port)
        except socket.error as e:
            logging.error(e)
            logging.error("Failed to bind honeytoken HTTP server to address %s on port %s" % (
                config.honeytoken_host, config.honeytoken_port))
            raise DceptError()
        ...
        logging.info("Testing connection to master node '%s'" %
                     (config.master_node))
    ...
    try:
        kerbsniff(config.interface, config.honey_username,
                  config.domain, config.realm)
...
if __name__ == "__main__":
    ...
    try:
        ...
        logging.basicConfig(filename='/opt/dcept/var/dcept.log',
