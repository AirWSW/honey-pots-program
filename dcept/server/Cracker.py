...
class Cracker:
    ...
    def enqueueJob(self, username, domain, encTimestamp, reqInfo, callback):
        self.passwordQueue.put((username, domain, encTimestamp, reqInfo, callback))
    ...
    # domain\username. 
    def recoverPassword(self, username, domain, encTimestamp, reqInfo, callback):
        ...
        logging.info("Cracking job completed")
        ...
        for line in lines:
            if line.endswith("(?)"):
                success = True
                password = line.split(" ")
                logging.info("Cracked! Password: %s" % (password[0]))
                callback(self.genServer, password[0], reqInfo)
