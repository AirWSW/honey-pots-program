...
class Cracker:
    ...
    def recoverPassword(self, username, domain, encTimestamp, callback):
        ...
        fh = open(passPath, 'w')
        fh.write("$%s$%d$%s$%s$$%s" %
                 ("krb5pa", 18, username, domain, encTimestamp))
        fh.close()
        ...
        if len(wordlist) == 0:
            logging.info(
                "Generation server hasn't issued any passwords. There is nothing to crack")
            return
        ...
        result = subprocess.check_output("/opt/dcept/john --wordlist=%s --pot=%s --format=krb5pa-sha1 %s %s" % (
